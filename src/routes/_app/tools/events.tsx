...
export const Route = createFileRoute("/_app/tools/events")({
  ...
  head: () => ({
    meta: [
      { title: "Nitrado Events â€” DayZ Pro" },
    ...
    <div className="space-y-6">
      {!hideHeader && (
        <header className="flex items-start gap-4">
          ...
          <div>
            <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Nitrado Â· Discord</div>
            <h1 className="mt-1 font-display text-3xl md:text-4xl">Events</h1>
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-glass-border bg-black/20 p-3">
        <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Server</span>
        {DAYZ_SERVERS.map((s) => (
          <button
            key={s.id}
            type="button"
          ...
          {catalog.map((ev) => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-1 text-[10px] text-muted-foreground">{ev.category}</div>
        ...
        <div className="lg:sticky lg:top-24 space-y-4">
          {selected ? (
            <GlassPanel className="border-primary/40 bg-black/95 p-5 shadow-xl">
              <div className="text-xs uppercase tracking-[0.18em] text-primary">Selected event</div>
              ...
              <label className="mt-4 block text-[10px] uppercase tracking-wider text-muted-foreground">
                ...
                <input
                  ...
                  placeholder="POI, duration, rulesâ€¦"
                />
              </label>
              <div className="mt-4 flex flex-col gap-2">
                ...
                >
                  {startMut.isPending ? "Startingâ€¦" : `Start on ${serverId}`}
                </button>
                <button
                  type="button"
                ...
                >
                  {stopMut.isPending ? "Stoppingâ€¦" : `Stop on ${serverId}`}
                </button>
              </div>
            </GlassPanel>
          ) : (
          ...
          <GlassPanel className="p-4">
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Active now</div>
            <div className="mt-2 space-y-2">
              ...
              {active.map((a) => (
                <div key={`${a.serverId}-${a.eventId}-${a.startedAt}`} className="text-sm">
                  <div className="font-medium">{a.eventName} Â· {a.serverId}</div>
