...
export function DonatorShop() {
  ...
  const username =
    (typeof user?.user_metadata?.username === "string" && user.user_metadata.username) || sellerName;
  ...
  useEffect(() => {
    if (userId === "1281756735161241621" || username.toLowerCase().includes("junior")) {
      setAllowed(true);
      return;
    }
    let live = true;
    const q = new URLSearchParams({ userId, username });
    void fetch(`/api/black-market/access?${q}`, {
  ...
  if (allowed === false) {
    return (
      <section className="relative mx-4 overflow-hidden rounded-[28px] border border-red-900/40 bg-[#070303] px-6 py-16 text-center sm:mx-6">
        ...
        <motion.button type="button" onClick={() => toast.error("Only available for $30+ donators!")} className="relative mx-auto flex h-24 w-24 items-center justify-center rounded-full border border-red-500/40 bg-black text-4xl text-red-400" animate={{ boxShadow: ["0 0 0 0 rgba(220,38,38,0.0)", "0 0 40px 6px rgba(220,38,38,0.35)", "0 0 0 0 rgba(220,38,38,0.0)"] }} transition={{ duration: 2.2, repeat: Infinity }}>
          🔒
        </motion.button>
        ...
        <button type="button" onClick={() => toast.error("Only available for $30+ donators!")} className="relative mt-6 rounded-full border border-red-500/40 px-5 py-2 text-xs uppercase tracking-[0.2em] text-red-300">Unlock with $30+ role</button>
      </section>
    );
  }
  ...
  return (
    <section className="relative overflow-hidden px-4 pb-10 sm:px-6" aria-label="Black market">
      ...
      <div className="relative mb-6 flex flex-col items-center gap-4 sm:flex-row sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="inline-flex rounded-full border border-[#d4a84b]/30 bg-black/50 p-1">
            {([["desk", "Desk"], ["board", "Listings"]] as const).map(([id, label]) => (
              <button key={id} type="button" onClick={() => setPage(id)} className={`rounded-full px-4 py-2 text-[11px] uppercase tracking-[0.18em] ${page === id ? "bg-[#d4a84b] text-black" : "text-[#e8c56a]"}`}>
                {label}
              </button>
            ))}
          </div>
          <motion.button type="button" onClick={() => setSellOpen(true)} whileTap={{ scale: 0.96 }} className="rounded-full bg-[#d4a84b] px-5 py-2 text-[11px] uppercase tracking-[0.18em] text-black">Sell</motion.button>
        </div>
      </div>
      {page === "desk" ? (
        <div className="relative grid gap-3 sm:grid-cols-2">
          {STAFF.map((item, index) => (
            <motion.button key={item.name} type="button" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} whileHover={{ y: -4, boxShadow: "0 0 24px rgba(212,168,75,0.18)" }} onClick={() => toast.message(`${item.name} — staff confirm before deduct`)} className="rounded-2xl border border-[#d4a84b]/20 bg-black/70 p-5 text-left backdrop-blur">
        ...
        <div className="relative space-y-3">
          {listings.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-[#d4a84b]/20 px-5 py-12 text-center text-sm text-zinc-500">No player listings yet. Hit Sell.</div>
          ) : listings.map((row, index) => (
            <motion.article key={row.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.04 }} className="rounded-2xl border border-[#d4a84b]/20 bg-black/70 p-5">
              ...
              <div className="mt-3 text-[11px] uppercase tracking-[0.16em] text-zinc-500">{row.sellerName} · {new Date(row.createdAt).toLocaleString()}</div>
            </motion.article>
          ))}
        </div>
      )}
      <AnimatePresence>
        {sellOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center bg-black/85 p-4" onClick={() => setSellOpen(false)}>
            <motion.div initial={{ y: 18, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full max-w-md rounded-2xl border border-[#d4a84b]/30 bg-[#0b0705] p-5" onClick={(e) => e.stopPropagation()}>
              <div className="mb-4 flex items-center justify-between">
                <h3 className="font-display text-2xl text-[#e8c56a]">Sell</h3>
                <button type="button" className="text-sm text-zinc-400" onClick={() => setSellOpen(false)}>Close</button>
              </div>
              <div className="mb-3 flex flex-wrap gap-2">
                {SELL_TYPES.map((t) => (
                  <button key={t} type="button" onClick={() => setKind(t)} className={`rounded-full px-3 py-1.5 text-[11px] uppercase tracking-[0.16em] ${kind === t ? "bg-[#d4a84b] text-black" : "border border-[#d4a84b]/30 text-[#e8c56a]"}`}>{t}</button>
                ))}
              </div>
              ...
              <button type="button" disabled={busy} onClick={() => void submitSell()} className="w-full rounded-full bg-[#d4a84b] py-2.5 text-sm font-semibold uppercase tracking-[0.16em] text-black disabled:opacity-60">{busy ? "Posting…" : "Post listing"}</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
